...
void generarSemillas() {
  ...
  for (int x = 0; x < width; x += paso) {
    for (int y = 0; y < height; y += paso) {
      ...
      boolean tieneColor = (r > 30 || g > 30 || b > 30);
      ...
      if (tieneColor) {
        ...
        velocidadesTallo.add(random(0.5, 1.5));
...
void draw() {
  ...
  for (int i = 0; i < todosLosTallos.size(); i++) {
    ...
    stroke(r, g, b, 100);
    strokeWeight(2);
    ellipse(inicio.x, inicio.y, 10 + sin(frameCount * 0.05 + i)*3, 10 + sin(frameCount * 0.05 + i)*3);
